...
export function Homepage() {
   ...
   const addToWatchList = async (event: MouseEvent) => {
      ...
      await axios
         .post(
            `http://localhost:8080/videoDB/addToWatchedList?userName=${userName}&videoID=${videoID}`
         )
         .then(() => {
            console.log(
               `Video id ${videoID} has been added to ${userName}'s watched list`
            );
         })
         .catch((err: any) => {
            console.log(
               `There was an error adding the video id ${videoID} to ${userName}'s watched list`
            );
         });
   };
   ...
   const removeFromWatchList = async (event: MouseEvent) => {
      ...
      await axios
         .post(
            `http://localhost:8080/videoDB/removeFromWatchedList?userName=${userName}&videoID=${videoID}`
         )
         .then(() => {
            console.log(
               `Video id ${videoID} has been added to ${userName}'s watched list`
   ...
   return (
      <>
         ...
         {topTV !== undefined ? (
            <Recommendations vedios={topTV.results} config={config} />
         ) : null}
         <TrendingVideos />
        ...
        <CarouselList vedioList={trendingMovie.results} config={config}/>: null} */}
         ...
         {topTV !== undefined ? (
            <YourWatchList watchList={topTV.results} config={config} />
         ) : null}
      </>
   );
}
